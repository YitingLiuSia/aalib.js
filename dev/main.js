...
let brightnessValue =
    brightnessEle.nextElementSibling.querySelector(".sliderValue");
let contrastValue =
    contrastEle.nextElementSibling.querySelector(".sliderValue");
...
let gradientAngleValue = gradientAngle.nextElementSibling.querySelector(
    "#gradient-angle .sliderValue"
);
let currentGradientAngle = 0;
let saturationContainer = document.getElementById("saturation");
let saturationForGradient =
    saturationContainer.getElementsByTagName("input")[0];
let saturationForGradientValue = document.querySelector(
...
let processedAssetCanvas =
    document.getElementById("processed-asset").children[0];
...
let videoInitialStatus =
    "Current status: click download video to start recording with current settings";
...
class PresetInfo {
    constructor(
        ...
        currentSaturationForGradient,
        currentimageExportRatio
    ) {
...
function processVideo(video) {
    ...
    const asciiDimensions = calculateAsciiDimensionsForImageSize(
        videoWidth,
        videoHeight,
        Number(fontSize.value),
        (Number(fontSize.value) / charWidthOffsetRatio) * lineHeightOffsetRatio
    );
    const aaReq = {
        width: asciiDimensions.width,
        height: asciiDimensions.height,
    };
    ...
    updateGradientFromCanvas(
        processedAssetCanvasCTX,
        currentGradientAngle,
        processedWidth,
        processedHeight
    );
    console.log(
        `processedAssetCanvasCTX dimension: ${processedWidth}x${processedHeight}`
    );
    ...
    console.log(
        `canvas dimension is ${canvasOptions.width}x${canvasOptions.height}`
    );
    ...
    if (inverseEle.checked) {
        videoProcessingPipeline = videoProcessingPipeline.map(
            aalib.filter.inverse()
        );
    }
    if (brightnessValue.value !== undefined) {
        videoProcessingPipeline = videoProcessingPipeline.map(
            aalib.filter.brightness(brightnessValue.value)
        );
    }
    if (contrastValue.value !== undefined) {
        videoProcessingPipeline = videoProcessingPipeline.map(
            aalib.filter.contrast(contrastValue.value)
        );
    }
...
function processImage(img) {
    return new Promise((resolve, reject) => {
        ...
        const asciiDimensions = calculateAsciiDimensionsForImageSize(
            ...
            imageHeight,
            Number(fontSize.value),
            (Number(fontSize.value) / charWidthOffsetRatio) *
                lineHeightOffsetRatio
        );
        const aaReq = {
            width: asciiDimensions.width,
            height: asciiDimensions.height,
        ...
        console.log(
            `processed dimension: ${processedWidth}x${processedHeight}`
        );
        ...
        if (inverseEle.checked) {
            imageProcessingPipeline = imageProcessingPipeline.map(
                aalib.filter.inverse()
            );
        }
        if (brightnessValue.value !== undefined) {
        ...
        if (isCanvas) {
            ...
            imageProcessingPipeline
                .map(aalib.render.html(canvasOptions))
...
function updateImageSizeWithWidth(img, newWidth) {
    ...
    const oldWidth = Number.parseFloat(
        getComputedStyle(img).width || img.getAttribute("width")
    );
    const oldHeight = Number.parseFloat(
        getComputedStyle(img).height || img.getAttribute("height")
    );
...
function clearCanvas() {
    ...
    if (processedAssetCanvas) {
        ctx.clearRect(
            0,
            0,
            processedAssetCanvas.width,
            processedAssetCanvas.height
        );
    }
}
...
function loadPresetFromFile(file) {
    ...
    reader.onload = function (event) {
        ...
        currentGradientAngle =
            data.currentGradientAngle || currentGradientAngle; // Use existing value as fallback
        currentSaturationForGradient =
            data.currentSaturationForGradient || currentSaturationForGradient; // Use existing value as fallback
        currentimageExportRatio =
            data.currentimageExportRatio || currentimageExportRatio; // Use existing value as fallback
...
function calculateAsciiDimensionsForImageSize(
    pixelWidth,
    pixelHeight,
    charPixelWidth,
    charPixelHeight
