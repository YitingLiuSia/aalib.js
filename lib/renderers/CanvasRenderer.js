...
var CanvasRenderer = exports.CanvasRenderer = function (_BaseRenderer) {
    ...
    function CanvasRenderer(options) {
        ...
        if (_this.options.gradient) {
            _this.createGradientWithPosition();
            // this.createGradient(); but the color has no position but index
            _this.color = _this.gradient; // Use gradient as the text color
        } else {
    ...
    _createClass(CanvasRenderer, [{
        ...
        value: function createGradient() {
            ...
            var gradient = this.ctx.createLinearGradient(0, 0, this.el.width, this.el.height);
            this.options.gradient.forEach(function (color, index) {
                gradient.addColorStop(index / (_this2.options.gradient.length - 1), color);
            });
            this.gradient = gradient;
        }
    }, {
        key: "createGradientWithPosition",
        value: function createGradientWithPosition() {
            var gradient = this.ctx.createLinearGradient(0, 0, this.el.width, this.el.height);
            // Assuming this.options.gradient is an array of objects like [{color: '#fff', position: 0.5}, ...]
            this.options.gradient.forEach(function (stop) {
